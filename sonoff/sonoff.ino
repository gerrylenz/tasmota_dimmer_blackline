...
void SetDevicePower(power_t rpower, uint32_t source)
{
  ...
  else if ((EXS_RELAY == my_module_type) || (EXS_Blackline == my_module_type)) {
...
void MqttShowState(void)
{
  ...
  ResponseAppend_P(PSTR(",\"" D_JSON_WIFI "\":{\"" D_JSON_AP "\":%d,\"" D_JSON_SSID "\":\"%s\",\"" D_JSON_BSSID "\":\"%s\",\"" D_JSON_CHANNEL "\":%d,\"" D_JSON_IPADDRESS "\":%s,\"" D_JSON_RSSI "\":%d,\"" D_JSON_LINK_COUNT "\":%d,\"" D_JSON_DOWNTIME "\":\"%s\"}}"),
    Settings.sta_active +1, Settings.sta_ssid[Settings.sta_active], WiFi.BSSIDstr().c_str(), WiFi.channel(), WiFi.localIP().toString().c_str(), WifiGetRssiAsQuality(WiFi.RSSI()), WifiLinkCount(), WifiDowntime().c_str());
...
//    devices_present = 1;
  ...
  for (uint32_t i = 0; i < MAX_RELAYS; i++) {
    if (pin[GPIO_REL1 +i] < 99) {
      ...
      devices_present++;
      if ((EXS_RELAY == my_module_type) || (EXS_Blackline == my_module_type)) {
